...
const MyClasses = ({ deleteClass }) => {
    ...
    const inviteFriends = (spinClass) => {
        selectedFriends.map((friend) => {
            ...
            addDoc(mailboxRef, {
                senderId: userId,
                content: `Come join me at ${
                    spinClass.instructor
                }'s class on ${spinClass.date
                    .toDate()
                    .toDateString()} ${spinClass.date
                    .toDate()
                    .toLocaleTimeString(undefined, {
                        hour: "numeric",
                        minute: "numeric",
                    })} at ${spinClass.location}. I will be on bike ${
                    spinClass.bike
                }.`,
            });
        });
        ...
        <div className="container mx-auto mt-6 px-8 py-5">
            ...
            {loading ? (
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
                    {classes
                        .filter((item) => item.date.toDate() >= new Date())
                        .map((classItem) => (
                            <ConditionalBadge
                                condition={isClassAddedInPastMinute(
                                    classItem.lastEdited
                                )}
                                wrapper={(children) => (
                            ...
                            >
                                ...
                                >
                                    <p>
                                        <strong>Date:</strong>{" "}
                                        {classItem.date.toDate().toDateString()}
                                    </p>
                                    <p>
                                        <strong>Time:</strong>{" "}
                                        {classItem.date
                                    ...
                                    <div className="flex justify-end mt-2">
                                        <button
                                            onClick={() =>
                                                togglePopup(classItem)
                                            }
                                            className="bg-green-500 hover:bg-green-600 text-black font-bold py-2 px-4 mr-1 rounded"
                                        >
                                            Invite
                                        </button>
                                        <div className="flex">
                                            <NavLink
                                                to={`/edit-class/${classItem.id}/${userId}`}
                                                className="bg-blue-500 hover:bg-blue-700 text-black font-bold py-2 px-4 mr-1 rounded"
                                            >
                                                Edit
                                            </NavLink>
                                        </div>
                                        <button
                                            onClick={() =>
                                                onDeleteClick(classItem.id)
                                            }
                                            className="bg-red-400 hover:bg-red-600 text-black font-bold py-2 px-4 rounded"
                                        >
                                            Delete
                                        </button>
                                    </div>
                ...
                <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center">
                    <div className="bg-white rounded-lg shadow-lg p-4 w-80">
                            ...
                            friendList.map((friend) => (
                                ...
                                >
                                    <div className="relative">
                                        <img
                                            src={friend.pic}
                                            className={`cursor-pointer w-12 h-12 rounded-full border ${
                                                selectedFriends.includes(
                                                    friend.id
                                                )
                                                    ? "border-blue-500 border-2"
                                                    : "border-gray-400"
                                            } object-cover mr-2 mb-2`}
                                            alt="profilepic"
                                        />
                                        {selectedFriends.includes(
                                            friend.id
                        ...
                        <button
                            ...
                            onClick={() => inviteFriends(currClass)}
                            className={`flex ml-auto py-2 px-2 rounded ${
                                selectedFriends.length === 0
                                    ? "cursor-not-allowed border-gray-300 bg-gray-200 text-gray-400"
                                    : "bg-blue-500 hover:bg-blue-700 text-black font-bold"
                            }`}
                        >
                            Confirm
                        </button>
