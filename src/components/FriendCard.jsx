...
const FriendCard = ({ currUserId, potentialFriend }) => {
  ...
  return (
    <div key={potentialFriend?.id} className="border border-black rounded p-4 mb-4 mr-4 flex flex-col items-center relative"> 
        ...
        {!buttonDisabled && <button onClick={() => sendRequest()} className="flex items-center border border-sky-500 bg-white text-blue-500 hover:bg-sky-100 font-semibold py-2 px-4 rounded-full">
            Add Friend
